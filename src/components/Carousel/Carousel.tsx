...
function Carousel() {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:3001/pets?limit=4')   // https://test-pet-foster-back.onrender.com/pets    --  https://aboubnan-server.eddi.cloud/pets
      .then((response) => {
        ...
        {animals.length === 0 
          ...
          <div className="carousel-wrapper">
            <div className="carousel-images">
              ...
              <SideItem key={animals[activeIndex - 1 < 0 ? animals.length - 1 : activeIndex - 1].name} animal={animals[activeIndex - 1 < 0 ? animals.length - 1 : activeIndex - 1]} direction={direction} />

              <Item key={animals[activeIndex].name} animal={animals[activeIndex]} activeIndex={activeIndex} setActiveIndex={setActiveIndex} direction={direction} setDirection={setDirection} />
              ...
              <SideItem key={animals[activeIndex + 1 >= animals.length ? 0 : activeIndex + 1].name} animal={animals[activeIndex + 1 >= animals.length ? 0 : activeIndex + 1]} direction={direction}  />
              {/* <div className="section-carousel-img">
                <img
                  src="src/assets/border-collie.jpg"
                  alt="chien heureux dans l'herbe"
            ...
            {/* Chevrons carousel */}
            <Arrows setDirection={setDirection} setActiveIndex={setActiveIndex} activeIndex={activeIndex} animals={animals} animal={animals[activeIndex]} />
