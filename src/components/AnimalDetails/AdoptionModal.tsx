...
const AdoptionModal =({closeModal, animal, setMessage}) => {
    ...
    const { register, handleSubmit, formState: { errors }} = useForm({resolver: zodResolver(requestCreateSchema)}); // utilisation de zodResolver et on indique le schéma utilisé

    // fonction pour la soumission du formulaire d'adoption
    const submitAdoptionForm = (dataSubmit) => {
        const formData = new FormData();
        for (const key in dataSubmit) {  // on met les données en FormData
            if (key === 'field') {
        ...
        if(formData.get("request_text") && formData.get("starting_date") && formData.get("ending_date") && formData.get("accept") &&  animal.association.id && animal.id ) {
            ...
            data = ({...data, status: "Nouveau", association_id: animal.association.id, pet_id: animal.id}); // on intègre les id en number donc on reprend les valeurs numériques (pour ne pas avoir de string lié au formData)

            console.log(data);

            axios.post("http://localhost:3001/requests",  JSON.stringify(data), {headers: {  
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem("token")}`  // on met le token dans l'entête Authorization
                }})
                .then(response => response.status === 201 ? setMessage("La demande d'adoption a bien été enregistrée") : "")
            ...
            console.log("erreur : le formulaire est incomplet ou avec des valeurs incorrectes")
        }
    }
    ...
    function calculateMinStartingDate(date) {
        ...
        return new Date(date_starting).toLocaleDateString().split("/").reverse().join("-");
    }
    ...
    function calculateMinEndingDate(date) {
        ...
        return new Date(date_ending).toLocaleDateString().split("/").reverse().join("-");
    }
        ...
        <section id="modal" className="close" onClick={closeModal}>
            <article>
                ...
                <form id="modal-form" onSubmit={handleSubmit(submitAdoptionForm)}>
                    <textarea  {...register("request_text")} name="request_text" id="request-text" placeholder={`Merci de saisir ici vos motivations pour l'accueil temporaire de "${animal.name}".\nVeuillez-indiquer également la composition de votre habitation (enfants, animaux, surface, jardin, etc...)`}>
                    </textarea>
                    
                    <fieldset>
                        <legend>Dates d'accueil temporaire</legend>
                    <div id="dates">
                        <label htmlFor="starting_date" className="date">Debut <input {...register("starting_date")} type="date" id="starting_date" name="starting_date" className="calendar" min={calculateMinStartingDate(new Date())} max={calculateMaxDate(new Date().toLocaleDateString().split("/").reverse().join("-"))} onChangeCapture={event => setBeginningDate(event.target.value)} /></label>
                        <label htmlFor="ending_date" className="date">Fin <input {...register("ending_date")} type="date" id="ending_date" name="ending_date" className="calendar" disabled={!beginningDate} min={calculateMinEndingDate(new Date(beginningDate))} max={calculateMaxDate(new Date().toLocaleDateString().split("/").reverse().join("-"))} /></label>
                    </div>
                    </fieldset>
                    <label htmlFor="accept" id="confirm-text"><input type="checkbox" {...register("accept")} name="accept" id="accept" checked={accept} onChange={() => setAccept(prev => !prev)} /> J'ai pris connaissance et accepte le règlement relatif à l'adoption temporaire d'un animal... Pour plus de renseignements sur l'adoption, merci de consulter la <Link  className="close" to="/contact" onClick={closeModal}>FAQ</Link></label>
                    
                    <button type="submit" className={`link-redirection ${accept ? "" : "disabled"}`} aria-disabled={accept ? false : true}  disabled={!accept} >
					    C'est parti !
					</button>
                    {/* bloc affichage des erreurs en validation */}
                    {errors !== null && <ul id="errors">
